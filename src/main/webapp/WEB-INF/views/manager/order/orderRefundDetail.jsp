...
<div class="container">
		...
		<form id = "refundForm" action = "refundSave" method = "post">
			<div id = "apply">
				<div id = "stateInfo">
					<span>접수일: <fmt:formatDate value = "${refund.refRequestDate}" pattern = "YYYY-MM-dd HH:mm:ss" /></span>
					<span>처리 완료일 : <c:choose><c:when test = "${refund.refCompleteDate!=null}">${refund.refCompleteDate}</</c:when><c:otherwise>-</c:otherwise></c:choose></span>
				...
				<div>
					<table id = "firstTable">
						...
						<tr>
							...
							<td colspan = "3">
								<span>${refund.user.userPhone}</span>
							</td>
						</tr>
						<tr>
							<td>						
							...
							<td colspan = "3">
								<span>${refund.goods[0].godName}
										<c:if test = "${(fn:length(refund.goods)-1) != 0}">
											외 ${fn:length(refund.goods)-1}건
										</c:if>
								</span>
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan = "3" id = "tdcontentWrite">
								<textarea id = "contentWrite" cols="90" rows="5" name = "content">${refund.refContent}</textarea>
							</td>
						</tr>
						<tr>
							<td>						
							...
							<td id = "imgtd" colspan = "3">
								<c:forEach var = "url"  items = "${refund.images}" varStatus = "state">
									<span>${url.refundImageUrl}</span><br>
								</c:forEach>
							</td>
						</tr>
						<tr>
					...
					<table id = "secondTable">
						<tr>
							...
							<td>
								<textarea name = "memo" cols="87" rows="5"placeholder= "처리 사항을 작성해주세요">${refund.refMemo}</textarea>
							</td>
						</tr>
					</table>
					<div id = "submitDiv">
	...
	$(function(){
		...
		$("#cancel").click(function(){
			Swal.fire({
				  ...
				  if (result.value) {
					window.location.href = "/shoppingmall/admin/orderRefund";
				  }
				})	
		})
		...
		$("#delete").click(function(){
			Swal.fire({
				  ...
				  if (result.value) {
					  $("#refundForm").attr("action","orderRefundDelete");
					  $("#refundForm").submit();
				  }
				})			
		})
