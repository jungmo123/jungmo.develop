...
<div class="container">
		...
		<div id = "AllContent">
			<div id = "menuBar">
				...
				<p id = "currentIdx">&#124; 주문관리 > 교환</p>
			</div>
			<div id = "search">
				<form id = "dateForm"  action = "Exchangesearch" method = "post">
					<span>신청일</span>
					<div id = "input-group">
						<div id = "input1">
						...
						<div id = "keyword">
							...
							<input class = "form-control" id = "textarea" type = "text" name = "Exchangesearch" />
			...
			<div id = "orderState">
				<span>&#124; 처리상태</span>
				<a href = "orderExchangeOne1">처리 전</a>
				<span  class = "Y">&#124;</span>
				<a href = "orderExchangeTwo1">처리 완료</a>
				<button id = "exchangeCheck" class = "btn btn-default download">처리완료 목록으로 이동</button>
			</div>
			<form id = "exchangeForm" action = "exchangeModify" method = "post">
			<div id = "resultbox">
				<table>
					<tr id = "Trheader">
						<th>선택</th>
						...
						<th>처리 상태</th>
								...
								<c:forEach  var = "exc" items= "${exchange}" varStatus = "state">
										...
										<td >${exc.excRequestDate}<br><fmt:formatDate value = "${exc.excRequestDate}" pattern = "HH:mm:ss" /></td>
										<td class = "itemNum">${exc.ordNum}</td>
										<td class = "itemName">${exc.goods[0].godName}
											<c:if test = "${(fn:length(exc.goods)-1) != 0}">
												외 ${fn:length(exc.goods)-1}건
											</c:if>
										</td>
				...
				<div>
					<ul class = "pagination">
						<c:if test = "${pageMaker.prev}">
							<li><a href = "orderExchange${type}${pageMaker.startPage-1}"><span class = "glyphicon glyphicon-chevron-left"></span></a>
						</c:if>
						
						<c:forEach begin = "${pageMaker.startPage}" end = "${pageMaker.endPage}" var = "idx">
							<li <c:out value = "${pageMaker.page.currentPage==idx ? 'class=active' : ''}"/>>
								<a href = "orderExchange${type}${idx}">${idx}</a>
							</li>
						</c:forEach>

						<c:if test = "${pageMaker.next}">
							<li><a href = "orderExchange${type}${pageMaker.endPage+1}"><span class = "glyphicon glyphicon-chevron-right"></span></a>
						</c:if>
					</ul>
				</div>		
			</div>
...
<script type = "text/javascript">
	$(".itemName").click(function(){
		...
		location.href="/shoppingmall/admin/ExchangeDetail" + name;
	})
	
	$("#exchangeCheck").click(function(e){
		$("#exchangeForm").submit();
	})
